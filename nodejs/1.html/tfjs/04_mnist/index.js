...
async function train(model, onIteration) {
  ...
  await model.fit(trainData.xs, trainData.labels, {
    ...
    callbacks: {
      onBatchEnd: async (batch, logs) => {
        trainBatchCount++
        ui.logStatus(`훈련... (` + `${((trainBatchCount / totalNumBatches) * 100).toFixed(1)}%` + ` 완료). 훈련을 멈추려면 페이지를 새로고침하거나 종료하세요.`)
  ...
  ui.logStatus(`최종 검증 정확도: ${finalValAccPercent.toFixed(1)}%; ` + `최종 테스트 정확도: ${testAccPercent.toFixed(1)}%`)
}
